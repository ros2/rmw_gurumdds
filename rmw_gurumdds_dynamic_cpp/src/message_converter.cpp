  ...
  { \
    if (member->is_array_) { \
      if (!member->array_size_ || member->is_upper_bound_) { \
        auto seq = \
          *(reinterpret_cast<const rosidl_generator_c__uint ## SIZE ## __Sequence *>(input + member->offset_)); \
        buffer << static_cast<const uint32_t>(seq.size); \
 \
        buffer.copy_arr(seq.data, seq.size); \
      } else { \
        for (uint32_t i = 0; i < member->array_size_; i++) { \
          buffer << *(reinterpret_cast<const uint ## SIZE ## _t *>(input + member->offset_) + i); \
        } \
        buffer.copy_arr( \
...
#define DESERIALIZER_C_DESERIALIZE_PRIMITIVE(SIZE) \
 ...
 \
        auto seq = \
          *(reinterpret_cast<rosidl_generator_c__uint ## SIZE ## __Sequence *>(output + member->offset_)); \
        if (seq.data) { \
          rosidl_generator_c__uint ## SIZE ## __Sequence__fini(&seq); \
        } \
        bool res = rosidl_generator_c__uint ## SIZE ## __Sequence__init(&seq, size); \
        if (!res) { \
  ...
  if (member->is_array_) {
    if (!member->array_size_ || member->is_upper_bound_) {
      ...
      for (uint32_t i = 0; i < member->size_function(input + member->offset_); i++) {
        buffer << *(reinterpret_cast<const uint8_t *>(member->get_const_function(input + member->offset_, i)));
      }
    }
  } else {
  ...
  const uint8_t * input)
{
  if (member->is_array_) {
    ...
    for (uint32_t i = 0; i < member->size_function(input + member->offset_); i++) {
      buffer << static_cast<const uint32_t>(*(reinterpret_cast<const uint16_t *>(member->get_const_function(input + member->offset_, i))));
    }
  } else {
    buffer << static_cast<const uint32_t>(*(reinterpret_cast<const uint16_t *>(input + member->offset_)));
  }
}
  ...
  const uint8_t * input)
{
  if (member->is_array_) {
    if (!member->array_size_ || member->is_upper_bound_) {
      ...
      // Array
      for (uint32_t i = 0; i < member->array_size_; i++) {
        buffer << *(reinterpret_cast<const rosidl_generator_c__String *>(input + member->offset_) + i);
      }
    }
  } else {
  ...
  const uint8_t * input)
{
  if (member->is_array_) {
    if (!member->array_size_ || member->is_upper_bound_) {
      // Sequence
      auto seq =
        *(reinterpret_cast<const rosidl_generator_c__U16String__Sequence *>(input + member->offset_));
      buffer << static_cast<const uint32_t>(seq.size);
      ...
      // Array
      for (uint32_t i = 0; i < member->array_size_; i++) {
        buffer << *(reinterpret_cast<const rosidl_generator_c__U16String *>(input + member->offset_) + i);
      }
    }
  } else {
  ...
  if (member->is_array_) {
    if (!member->array_size_ || member->is_upper_bound_) {
      ...
      for (uint32_t i = 0; i < size; i++) {
        uint8_t data = 0;
        buffer >> data;
        vec[i] = static_cast<bool>(data != 0);
      }
    } else {
      // Array
      for (uint32_t i = 0; i < member->array_size_; i++) {
        buffer >> *(reinterpret_cast<uint8_t *>(member->get_function(output + member->offset_, i)));
        uint8_t data = 0;
        buffer >> data;
        *(reinterpret_cast<bool *>(member->get_function(output + member->offset_, i))) = static_cast<bool>(data != 0);
      }
    }
  } else {
    uint8_t data = 0;
  ...
  uint8_t * output)
{
  if (member->is_array_) {
    ...
    for (uint32_t i = 0; i < member->size_function(output + member->offset_); i++) {
      uint32_t data = 0;
      buffer >> data;
      *(reinterpret_cast<uint16_t *>(member->get_function(output + member->offset_, i))) = static_cast<uint16_t>(data);
    }
  } else {
    uint32_t data = 0;
    buffer >> data;
  ...
  uint8_t * output)
{
  if (member->is_array_) {
    ...
    for (uint32_t i = 0; i < member->size_function(output + member->offset_); i++) {
      buffer >> *(reinterpret_cast<std::string *>(member->get_function(output + member->offset_, i)));
    }
  } else {
  ...
  uint8_t * output)
{
  if (member->is_array_) {
    ...
    for (uint32_t i = 0; i < member->size_function(output + member->offset_); i++) {
      buffer >> *(reinterpret_cast<std::u16string *>(member->get_function(output + member->offset_, i)));
    }
  } else {
  ...
  uint8_t * output)
{
  if (member->is_array_) {
    if (!member->array_size_ || member->is_upper_bound_) {
      ...
      auto seq =
        *(reinterpret_cast<const rosidl_generator_c__boolean__Sequence *>(output + member->offset_));
      if (seq.data) {
        rosidl_generator_c__boolean__Sequence__fini(&seq);
      }
      bool res = rosidl_generator_c__boolean__Sequence__init(&seq, size);
      if (!res) {
        throw std::runtime_error("Failed to initialize sequence");
      }
  ...
  uint8_t * output)
{
  if (member->is_array_) {
    if (!member->array_size_ || member->is_upper_bound_) {
      ...
      auto seq =
        *(reinterpret_cast<const rosidl_generator_c__wchar__Sequence *>(output + member->offset_));
      if (seq.data) {
        rosidl_generator_c__wchar__Sequence__fini(&seq);
      }
      bool res = rosidl_generator_c__wchar__Sequence__init(&seq, size);
      if (!res) {
        throw std::runtime_error("Failed to initialize sequence");
      }
      ...
      for (uint32_t i = 0; i < size; i++) {
        uint32_t data = 0;
        buffer >> data;
        seq.data[i] = static_cast<uint16_t>(data);
      }
    } else {
  ...
  uint8_t * output)
{
  if (member->is_array_) {
    if (!member->array_size_ || member->is_upper_bound_) {
      ...
      auto seq =
        *(reinterpret_cast<const rosidl_generator_c__String__Sequence *>(output + member->offset_));
      if (seq.data) {
        rosidl_generator_c__String__Sequence__fini(&seq);
      }
      bool res = rosidl_generator_c__String__Sequence__init(&seq, size);
      if (!res) {
        throw std::runtime_error("Failed to initialize sequence");
      }

      for (uint32_t i = 0; i < size; i++) {
        if (seq.data[i].data == nullptr) {
          if (!rosidl_generator_c__String__init(&seq.data[i])) {
  ...
  uint8_t * output)
{
  if (member->is_array_) {
    if (!member->array_size_ || member->is_upper_bound_) {
      ...
      auto seq =
        *(reinterpret_cast<const rosidl_generator_c__U16String__Sequence *>(output + member->offset_));
      if (seq.data) {
        rosidl_generator_c__U16String__Sequence__fini(&seq);
      }
      bool res = rosidl_generator_c__U16String__Sequence__init(&seq, size);
      if (!res) {
        throw std::runtime_error("Failed to initialize sequence");
      }
      ...
      for (uint32_t i = 0; i < size; i++) {
        if (seq.data[i].data == nullptr) {
          if (!rosidl_generator_c__U16String__init(&seq.data[i])) {
            throw std::runtime_error("Failed to initialize string");
          }
        }
        buffer >> seq.data[i];
