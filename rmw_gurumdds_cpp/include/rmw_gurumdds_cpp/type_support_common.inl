...
template <typename MessageMembersT>
void *allocate_message(const void *untyped_members, const uint8_t *ros_message,
                       size_t *size) {
  auto members = static_cast<const MessageMembersT *>(untyped_members);
  if (nullptr == members) {
    RMW_SET_ERROR_MSG("Members handle is null");
  ...
  rmw_gurumdds_cpp::MessageSerializer<false, MessageMembersT> serializer{
      buffer};
...
template <typename MessageMembersT>
std::string parse_struct(const MessageMembersT *members,
  ...
  metastring << "{("
             ...
             << "type=" << create_type_name<MessageMembersT>(members)
             << ",member=" << members->member_count_ << ")";
  ...
  for (size_t i = 0; i < members->member_count_; i++) {
    ...
    if (member->is_array_) {
      if (member->array_size_ > 0) {
        if (!member->is_upper_bound_) {
          // Array
          metastring << "[(name=" << member->name_
                     << "_,dimension=" << member->array_size_ << ")";
        } else {
          // BoundedSequence
          metastring << "<(name=" << member->name_
                     << "_,maximum=" << member->array_size_ << ")";
        }
      } else {
        // UnboundedSequence
    ...
    if (member->type_id_ ==
      ...
      auto inner_struct =
          static_cast<const MessageMembersT *>(member->members_->data);
      ...
      metastring << "("
                 << (member->is_array_
                         ? ""
                         : std::string("name=") + member->name_ + "_")
                 << ")";
    }
  }
