...
inline MessageSerializer<SERIALIZE, MessageMembersT>::MessageSerializer(
...
inline void MessageSerializer<SERIALIZE, MessageMembersT>::serialize(
    const MessageMembersT *members, const uint8_t *input, bool roundup_) {
...
inline void MessageSerializer<SERIALIZE, MessageMembersT>::serialize_boolean(
    const MessageMemberT *member, const uint8_t *input) {
  if (member->is_array_) {
    if constexpr (LANGUAGE_KIND == LanguageKind::C) {
      if (!member->array_size_ || member->is_upper_bound_) {
        // Sequence
        auto seq =
            *(reinterpret_cast<const rosidl_runtime_c__boolean__Sequence *>(
        ...
        for (uint32_t i = 0; i < member->array_size_; i++) {
          buffer << static_cast<uint8_t>(
              (reinterpret_cast<const bool *>(input + member->offset_))[i] ==
              true);
        }
      }
    }
    ...
    if constexpr (LANGUAGE_KIND == LanguageKind::CXX) {
      if (!member->array_size_ || member->is_upper_bound_) {
        // Sequence
        auto vec = *(reinterpret_cast<const std::vector<bool> *>(
            input + member->offset_));
    ...
    buffer << static_cast<uint8_t>(
        *(reinterpret_cast<const bool *>(input + member->offset_)));
  }
}
  ...
  if (member->is_array_) {
    if constexpr (LANGUAGE_KIND == LanguageKind::C) {
      if (!member->array_size_ || member->is_upper_bound_) {
        // Sequence
        auto seq =
            *(reinterpret_cast<const rosidl_runtime_c__wchar__Sequence *>(
                input + member->offset_));
        buffer << static_cast<uint32_t>(seq.size);
    ...
    if constexpr (LANGUAGE_KIND == LanguageKind::CXX) {
      const uint32_t size =
          static_cast<uint32_t>(member->size_function(input + member->offset_));
      if (!member->array_size_ || member->is_upper_bound_) {
        // Sequence
        buffer << size;
      }
      ...
      for (uint32_t i = 0; i < size; i++) {
        buffer << *(reinterpret_cast<const uint16_t *>(
            member->get_const_function(input + member->offset_, i)));
      }
    }
  } else {
...
inline void MessageSerializer<SERIALIZE, MessageMembersT>::serialize_string(
  ...
  if constexpr (LANGUAGE_KIND == LanguageKind::C) {
    if (member->is_array_) {
      if (!member->array_size_ || member->is_upper_bound_) {
        // Sequence
        auto seq =
            *(reinterpret_cast<const rosidl_runtime_c__String__Sequence *>(
                input + member->offset_));
        buffer << static_cast<uint32_t>(seq.size);
        ...
        // Array
        for (uint32_t i = 0; i < member->array_size_; i++) {
          buffer << *(reinterpret_cast<const rosidl_runtime_c__String *>(
                          input + member->offset_) +
                      i);
        }
      }
  ...
  if constexpr (LANGUAGE_KIND == LanguageKind::CXX) {
    if (member->is_array_) {
      ...
      for (uint32_t i = 0; i < size; i++) {
        buffer << *(reinterpret_cast<const std::string *>(
            member->get_const_function(input + member->offset_, i)));
      }
    } else {
      buffer << *(
          reinterpret_cast<const std::string *>(input + member->offset_));
    }
  }
}
...
inline void MessageSerializer<SERIALIZE, MessageMembersT>::serialize_wstring(
  ...
  if constexpr (LANGUAGE_KIND == LanguageKind::C) {
    if (member->is_array_) {
      if (!member->array_size_ || member->is_upper_bound_) {
        ...
        // Array
        for (uint32_t i = 0; i < member->array_size_; i++) {
          buffer << *(reinterpret_cast<const rosidl_runtime_c__U16String *>(
                          input + member->offset_) +
                      i);
        }
      }
  ...
  if constexpr (LANGUAGE_KIND == LanguageKind::CXX) {
    if (member->is_array_) {
      ...
      for (uint32_t i = 0; i < size; i++) {
        buffer << *(reinterpret_cast<const std::u16string *>(
            member->get_const_function(input + member->offset_, i)));
      }
    } else {
      buffer << *(
          reinterpret_cast<const std::u16string *>(input + member->offset_));
    }
  }
}
  ...
  if (member->is_array_) {
    ...
    for (uint32_t i = 0; i < size; i++) {
      serialize(
          ...
          reinterpret_cast<const uint8_t *>(member->get_const_function(ptr, i)),
          false);
    }
  } else {
    serialize(reinterpret_cast<const MessageMembersT *>(member->members_->data),
              reinterpret_cast<const uint8_t *>(input + member->offset_),
              false);
  }
}
...
inline void MessageSerializer<SERIALIZE, MessageMembersT>::serialize_primitive(
    const MessageSerializer::MessageMemberT *member, const uint8_t *input) {
  if (member->is_array_) {
    if constexpr (LANGUAGE_KIND == LanguageKind::C) {
      if (!member->array_size_ || member->is_upper_bound_) {
        // Sequence
        auto seq = reinterpret_cast<const rmw_gurumdds_cpp::rmw_seq_t<T> *>(
            input + member->offset_);
        buffer << static_cast<uint32_t>(seq->size);
        buffer.copy_arr(seq->data, seq->size);
      } else {
        // Array
        buffer.copy_arr(reinterpret_cast<const T *>(input + member->offset_),
                        member->array_size_);
      }
    }
    ...
    if constexpr (LANGUAGE_KIND == LanguageKind::CXX) {
      ...
      buffer.copy_arr(reinterpret_cast<const T *>(member->get_const_function(
                          input + member->offset_, 0)),
                      size);
    }
