...
{
  CHECK_ALL_PTRS_NULL(context, name, namespace_);
  RMW_CHECK_FOR_NULL_WITH_MSG(
    context->impl, "expected initialized context", return nullptr);
  ...
  if (ret != RMW_RET_OK) {
    RCUTILS_LOG_ERROR_NAMED(
      RMW_GURUMDDS_ID, "failed to initialize node in context");
    return nullptr;
  }

  auto context_finalize = rcpputils::make_scope_exit([ctx]() {
  ...
  node_handle->namespace_ = static_cast<const char *>(
  ...
  std::memcpy(
    const_cast<char *>(node_handle->namespace_),
    namespace_,
    strlen(namespace_) + 1);
  ...
  if (
    rmw_gurumdds_cpp::graph_cache::on_node_created(ctx, node_handle) !=
    RMW_RET_OK)
  {
  ...
  RCUTILS_LOG_DEBUG_NAMED(
    RMW_GURUMDDS_ID, "Created node '%s' in namespace '%s'", name, namespace_);
...
{
  RMW_CHECK_FOR_NULL_WITH_MSG(
    node, "expected initialized node", return nullptr);
  auto common_ctx = &node->context->impl->common_ctx;
  RMW_CHECK_FOR_NULL_WITH_MSG(
    common_ctx, "expected initialized common_ctx", return nullptr);
...
{
  CHECK_ALL_PTRS_CODE(node, node_names, node_namespaces);
  CHECK_ID_CODE(node);
...
{
  CHECK_ALL_PTRS_CODE(node, node_names, node_namespaces, enclaves);
  CHECK_ID_CODE(node);
