...
create_publisher(
  ...
  const bool internal)
{
  CHECK_ALL_PTRS_NULL(ctx, participant, pub, type_supports, qos_policies, publisher_options);
  ...
  dds_typesupport = create_type_support_and_register(participant, gurum_type_support, type_name,
                                                     metastring);
  ...
  if (has_buffer_fields) {
    auto * backend_context =
      static_cast<BufferBackendContext *>(
      ctx->buffer_serialization_context);
    if (backend_context) {
  ...
  if (has_buffer_fields) {
    ...
    publisher_info->local_endpoint_info = rmw_get_zero_initialized_topic_endpoint_info();
    publisher_info->local_endpoint_info.endpoint_type = RMW_ENDPOINT_PUBLISHER;
    std::memcpy(
      publisher_info->local_endpoint_info.endpoint_gid,
    ...
    // cpu 토픽 네임은 fastrtps껄로 만들었으나, 문제 생기면 gurumdds 토픽 네임으로 변경할 것.
    std::string cpu_topic_name = fastrtps_topic_name_mangled + "/_buf_cpu";

    topic_desc = dds_DomainParticipant_lookup_topicdescription(
      participant, cpu_topic_name.c_str());
    if (topic_desc == nullptr) {
      dds_TopicQos cpu_topic_qos;
      ret = dds_DomainParticipant_get_default_topic_qos(participant, &cpu_topic_qos);
      if (ret != dds_RETCODE_OK) {
        RMW_SET_ERROR_MSG("failed to get default topic qos");
        return nullptr;
      }
      ...
      publisher_info->cpu_topic = dds_DomainParticipant_create_topic(
        participant, cpu_topic_name.c_str(), type_name.c_str(), &cpu_topic_qos, nullptr, 0);
      ...
      ret = dds_TopicQos_finalize(&cpu_topic_qos);
      if (ret != dds_RETCODE_OK) {
        RMW_SET_ERROR_MSG("failed to finalize topic qos");
        return nullptr;
      }
    ...
    if (!publisher_info->cpu_data_writer) {
      ...
      dds_DomainParticipant_delete_topic(participant, publisher_info->cpu_topic);
    ...
    RCUTILS_LOG_DEBUG_NAMED(
      RMW_GURUMDDS_ID,
      "Created buffer-aware publisher on '%s'", cpu_topic_name.c_str());
  }

  ///////////////////////////////////////////////////////////////////
  init_backend_buffer_publisher(publisher_info, node, topic_name);
  ...
  if (publisher_info->is_buffer_aware) {
    ...
    for (auto & endpoint : endpoints_to_destroy) {
      if (endpoint->data_writer) {
        ret = dds_Publisher_delete_datawriter(ctx->publisher, endpoint->data_writer);
        if (ret != dds_RETCODE_OK) {
      ...
      if (endpoint->topic && endpoint->owns_topic) {
        ret = dds_DomainParticipant_delete_topic(ctx->participant, endpoint->topic);
        if (ret != dds_RETCODE_OK) {
    ...
    if (publisher_info->cpu_data_writer) {
      ret = dds_Publisher_delete_datawriter(ctx->publisher, publisher_info->cpu_data_writer);
      if (ret != dds_RETCODE_OK) {
        RMW_SET_ERROR_MSG("failed to delete datawriter");
        return RMW_RET_ERROR;
      }
    ...
    if (publisher_info->cpu_topic) {
      ret = dds_DomainParticipant_delete_topic(ctx->participant, publisher_info->cpu_topic);
      if (ret != dds_RETCODE_OK) {
        RMW_SET_ERROR_MSG("failed to delete topic");
        return RMW_RET_ERROR;
      }
  ...
  rmw_publisher_allocation_t * allocation)
{
  ...
  const size_t ser_size =
    callbacks->get_serialized_size(ros_message);
...
void
create_pending_buffer_writers(PublisherInfo * info)
{
  ...
  for (auto & p : pending) {
    ...
    if (topic_desc == nullptr) {
      ...
      timeout.sec = 0;
      timeout.nanosec = 1;
      topic = dds_DomainParticipant_find_topic(info->participant, p.unique_topic.c_str(), &timeout);
      if (topic == nullptr) {
        RMW_SET_ERROR_MSG("failed to find topic");
        continue;
      }
    ...
    dds_DataWriter_get_qos(info->topic_writer, &writer_qos);
    //fastrtps에서는 동기 publish 모드, PREALLOCATED_WITH)_REALLOC_MEMORY_MODE, 데이터 공유 off로 고정하여 사용함.
    {
  ...
  if (!state.cpu_only_subscribers.empty() && info->cpu_data_writer) {
    ...
    try {
      ...
      RCUTILS_LOG_ERROR_NAMED(
        ...
        "backend-buffer path, but this subscription is currently receiving through the CPU path. "
        "Please check the publisher/subscription backend-buffer configuration and ensure that "
...
rmw_create_publisher(
  ...
  const rmw_publisher_options_t * publisher_options)
{
  CHECK_ALL_PTRS_NULL(node, type_supports, topic_name, qos_policies, publisher_options);
  ...
  rmw_publisher_allocation_t * allocation)
{
  ...
  if(info->is_buffer_aware) {
    ...
    if (total_matched <= buffer_aware_count) {
      return rmw_gurumdds_cpp::publish_to_buffer_endpoint(publisher, ros_message, allocation);
    }

    //fallback
    return rmw_gurumdds_cpp::publish(publisher, ros_message,
                                   allocation);
  }
