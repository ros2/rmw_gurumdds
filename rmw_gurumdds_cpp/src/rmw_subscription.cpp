  ...
  const rmw_subscription_options_t * subscription_options)
{
  ...
  const rosidl_message_type_support_t * type_support =
    get_message_typesupport_handle(type_supports, RMW_GURUMDDS_CPP_TYPESUPPORT_C);
  if (type_support == nullptr) {
    type_support = get_message_typesupport_handle(type_supports, RMW_GURUMDDS_CPP_TYPESUPPORT_CPP);
    if (type_support == nullptr) {
...
rmw_destroy_subscription(rmw_node_t * node, rmw_subscription_t * subscription)
{
  ...
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(
    node handle, node->implementation_identifier, gurum_gurumdds_identifier, return RMW_RET_ERROR)
  ...
  rmw_subscription_allocation_t * allocation)
{
  ...
  return _take(gurum_gurumdds_identifier, subscription, ros_message, taken, nullptr, allocation);
}
  ...
  rmw_subscription_allocation_t * allocation)
{
  ...
  GurumddsSubscriberInfo * info = static_cast<GurumddsSubscriberInfo *>(subscription->data);
  RCUTILS_CHECK_FOR_NULL_WITH_MSG(info, "subscriber info is null", return RMW_RET_ERROR);
  ...
  *taken = 0;
  ...
  for (uint32_t i = 0; i < dds_SampleInfoSeq_length(sample_infos); i++) {
    ...
    if (!ignore_sample) {
      ...
      if (!info->callbacks->convert_dds_to_ros(sample, message_sequence->data[*taken])) {
        RMW_SET_ERROR_MSG("failed to convert message");
        dds_DataReader_return_loan(topic_reader, data_values, sample_infos);
        dds_DataSeq_delete(data_values);
        dds_SampleInfoSeq_delete(sample_infos);
  ...
  rmw_subscription_allocation_t * allocation)
{
  ...
  *taken = false;
  ...
  const char * topic_name =
    dds_TopicDescription_get_name(dds_DataReader_get_topicdescription(topic_reader));
  RCUTILS_LOG_DEBUG_NAMED(
    "rmw_gurumdds_cpp", "Received serialized data on topic %s", topic_name);
  ...
  rmw_subscription_allocation_t * allocation)
{
  ...
  return _take_serialized(
    gurum_gurumdds_identifier, subscription, serialized_message, taken, nullptr, allocation);
}
  ...
  rmw_subscription_allocation_t * allocation)
{
  ...
  return _take_serialized(
    gurum_gurumdds_identifier, subscription, serialized_message, taken, message_info, allocation);
}
