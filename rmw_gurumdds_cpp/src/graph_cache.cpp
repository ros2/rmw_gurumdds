...
static rmw_ret_t
add_entity(
  rmw_context_impl_t *ctx, const rmw_gid_t *const endp_gid,
  const rmw_gid_t *const dp_gid, const char *const topic_name,
  const char *const type_name, const rosidl_type_hash_s & type_hash,
  ...
  const dds_UserDataQosPolicy *const userdata, const bool is_reader,
  const bool local)
{
  ...
  const uint32_t *const dp_gid_32_arr =
    reinterpret_cast<const uint32_t *>(dp_gid->data);
  const uint32_t *const endp_gid_32_arr =
    reinterpret_cast<const uint32_t *>(endp_gid->data);
  RCUTILS_LOG_DEBUG_NAMED(
      ...
      reinterpret_cast<void *>(&ctx->common_ctx.graph_cache), dp_gid_32_arr[0],
      dp_gid_32_arr[1], dp_gid_32_arr[2], dp_gid_32_arr[3], endp_gid_32_arr[0],
      endp_gid_32_arr[1], endp_gid_32_arr[2], endp_gid_32_arr[3], topic_name,
      type_name, is_reader, local);
  ...
  std::string ud;
  if (userdata->size > 0) {
    ud.assign(reinterpret_cast<const char *>(userdata->value), userdata->size);
  }

  bool add_res = ctx->common_ctx.graph_cache.add_entity(
      *endp_gid, std::string(topic_name), std::string(type_name), type_hash,
      *dp_gid, qos_profile, is_reader, ser_type_hash_ptr);
  ...
  if (!add_res) {
    RCUTILS_LOG_DEBUG_NAMED(RMW_GURUMDDS_ID,
                            ...
                            "topic=%s, "
                            "type=%s",
                            endp_gid_32_arr[0], endp_gid_32_arr[1],
                            endp_gid_32_arr[2], endp_gid_32_arr[3], topic_name,
                            type_name);
...
static rmw_ret_t remove_entity(
  ...
  const rmw_gid_t gid, const bool is_reader)
{
  ...
  const rmw_gid_t gid)
{
  ...
  if (dds_RETCODE_OK != raii::dds_DataWriter_get_qos(datawriter, dw_qos)) {
    RMW_SET_ERROR_MSG("failed to get DataWriterQos");
    return RMW_RET_ERROR;
  }

  return add_entity(ctx, &gid, &ctx->common_ctx.gid, topic_name, type_name,
                    type_hash, &dw_qos->history, &dw_qos->reliability,
                    &dw_qos->durability, &dw_qos->deadline, &dw_qos->liveliness,
                    &dw_qos->lifespan, &dw_qos->user_data, false, true);
}
  ...
  const rmw_gid_t gid)
{
  ...
  raii::dds_DataReaderQos dr_qos;
  // dds_DataReaderQos dr_qos;

  auto topic = reinterpret_cast<dds_Topic *>(
    dds_DataReader_get_topicdescription(datareader));
  const char *topic_name = dds_Topic_get_name(topic);
  const char *type_name = dds_Topic_get_type_name(topic);

  if (dds_RETCODE_OK != raii::dds_DataReader_get_qos(datareader, dr_qos)) {
    RMW_SET_ERROR_MSG("failed to get DataReaderQos");
    return RMW_RET_ERROR;
  }

  return add_entity(ctx, &gid, &ctx->common_ctx.gid, topic_name, type_name,
                    type_hash, &dr_qos->history, &dr_qos->reliability,
                    &dr_qos->durability, &dr_qos->deadline, &dr_qos->liveliness,
                    nullptr, &dr_qos->user_data, true, true);
}

namespace rmw_gurumdds_cpp::graph_cache
{
rmw_ret_t initialize(rmw_context_impl_t *const ctx)
{
  ...
  ctx->common_ctx.pub = rmw_gurumdds_cpp::create_publisher(
      ctx, nullptr, ctx->participant, ctx->publisher, type_supports_partinfo,
      topic_name_partinfo, &qos, &publisher_options, true);
  ...
  ctx->common_ctx.sub = create_subscription(
      ctx, nullptr, ctx->participant, ctx->subscriber, type_supports_partinfo,
      topic_name_partinfo, &qos, &subscription_options, true);

  if (ctx->common_ctx.sub == nullptr) {
    RCUTILS_LOG_ERROR_NAMED(
        RMW_GURUMDDS_ID,
        "failed to create subscription for ParticipantEntityInfo");
    return RMW_RET_ERROR;
  }

  ctx->common_ctx.graph_guard_condition = rmw_create_guard_condition(ctx->base);
  ...
  ctx->common_ctx.publish_callback = [](const rmw_publisher_t *pub,
    const void *msg) {
      return rmw_gurumdds_cpp::publish(pub, msg, nullptr);
    };

  entity_get_gid(reinterpret_cast<dds_Entity *>(ctx->participant),
                 ctx->common_ctx.gid);
...
rmw_ret_t finalize(rmw_context_impl_t *const ctx)
{
  ...
  if (ctx->common_ctx.sub != nullptr) {
    if (RMW_RET_OK != destroy_subscription(ctx, ctx->common_ctx.sub)) {
...
rmw_ret_t enable(rmw_context_t *const ctx)
{
...
rmw_ret_t publish_update(rmw_context_impl_t *const ctx, void *const msg)
{
  if (ctx->common_ctx.pub == nullptr) {
...
rmw_ret_t on_node_created(
  rmw_context_impl_t *const ctx,
  const rmw_node_t *const node)
{
  rmw_ret_t rc = ctx->common_ctx.add_node_graph(node->name, node->namespace_);
  if (RMW_RET_OK != rc) {
...
rmw_ret_t on_node_deleted(
  rmw_context_impl_t *const ctx,
  const rmw_node_t *const node)
{
  rmw_ret_t rc =
    ctx->common_ctx.remove_node_graph(node->name, node->namespace_);
  if (RMW_RET_OK != rc) {
...
rmw_ret_t on_publisher_created(
  rmw_context_impl_t *const ctx,
  const rmw_node_t *const node,
  PublisherInfo *const pub)
{
  ...
  rmw_ret_t rc = add_local_publisher(ctx, node, pub->topic_writer, type_hash,
                                     pub->publisher_gid);
  if (RMW_RET_OK != rc) {
  ...
  rc = ctx->common_ctx.add_publisher_graph(pub->publisher_gid, node->name,
                                           node->namespace_);
  if (RMW_RET_OK != rc) {
...
rmw_ret_t on_publisher_deleted(
  rmw_context_impl_t *const ctx,
  const rmw_node_t *const node,
  PublisherInfo *const pub)
{
  ...
  rc = ctx->common_ctx.remove_publisher_graph(pub->publisher_gid, node->name,
                                              node->namespace_);
  if (RMW_RET_OK != rc) {
...
rmw_ret_t on_subscriber_created(
  rmw_context_impl_t *const ctx,
  const rmw_node_t *const node,
  SubscriberInfo *const sub)
{
  ...
  auto & type_hash = *msg_typesupport->get_type_hash_func(msg_typesupport);
  rmw_ret_t rc = add_local_subscriber(ctx, node, sub->topic_reader, type_hash,
                                      sub->subscriber_gid);
  if (RMW_RET_OK != rc) {
  ...
  rc = ctx->common_ctx.add_subscriber_graph(sub->subscriber_gid, node->name,
                                            node->namespace_);
  if (RMW_RET_OK != rc) {
...
rmw_ret_t on_subscriber_deleted(
  rmw_context_impl_t *const ctx,
  const rmw_node_t *const node,
  SubscriberInfo *const sub)
{
  ...
  rc = ctx->common_ctx.remove_subscriber_graph(sub->subscriber_gid, node->name,
                                               node->namespace_);
  if (RMW_RET_OK != rc) {
  ...
  ServiceInfo *const svc)
{
  ...
  if (RMW_RET_OK != add_local_subscriber(ctx, node, svc->request_reader,
                                         *type_hash, sub_gid))
  {
    RMW_SET_ERROR_MSG("failed to add local subscriber");
    return RMW_RET_ERROR;
  ...
  type_hash = type_support->get_type_hash_func(type_support);
  if (RMW_RET_OK != add_local_publisher(ctx, node, svc->response_writer,
                                        *type_hash, pub_gid))
  {
    RMW_SET_ERROR_MSG("failed to add local publisher");
    return RMW_RET_ERROR;
  ...
  ClientInfo *const client)
{
  ...
  type_hash = type_support->get_type_hash_func(type_support);
  if (RMW_RET_OK != add_local_subscriber(ctx, node, client->response_reader,
                                         *type_hash, sub_gid))
  {
    RMW_SET_ERROR_MSG("failed to add local subscriber");
    return RMW_RET_ERROR;
  ...
  type_hash = type_support->get_type_hash_func(type_support);
  if (RMW_RET_OK != add_local_publisher(ctx, node, client->request_writer,
                                        *type_hash, pub_gid))
  {
    RMW_SET_ERROR_MSG("failed to add local publisher");
    return RMW_RET_ERROR;
  ...
  if (RMW_RET_OK != ctx->common_ctx.add_client_graph(
                        pub_gid, sub_gid, node->name, node->namespace_))
  {
  ...
  ClientInfo *const client)
{
  ...
  rc = ctx->common_ctx.remove_client_graph(client->publisher_gid,
                                           client->subscriber_gid, node->name,
                                           node->namespace_);
  failed = failed || (RMW_RET_OK != rc);
...
rmw_ret_t on_participant_info(rmw_context_impl_t *ctx)
{
  bool taken = false;
  ...
  const char *const enclave)
{
  ...
  if (0 ==
    std::memcmp(gid.data, ctx->common_ctx.gid.data, RMW_GID_STORAGE_SIZE))
  {
...
rmw_ret_t add_remote_entity(
  rmw_context_impl_t *ctx,
                            // const dds_GUID_t * const endp_guid,
                            // const dds_GUID_t * const dp_guid,
  const rmw_gid_t & endp_gid, const rmw_gid_t & dp_gid,
...
{
  if (0 == std::memcmp(dp_gid.data, ctx->common_ctx.gid.data,
  ...
  if (RMW_RET_OK != add_entity(ctx, &endp_gid, &dp_gid, topic_name, type_name,
                               type_hash, nullptr, reliability, durability,
                               deadline, liveliness, lifespan, &user_data,
                               is_reader, false))
  {
    return RMW_RET_ERROR;
  }
...
rmw_ret_t remove_entity(
  rmw_context_impl_t *const ctx,
  const dds_GUID_t *const guid, const bool is_reader)
{
  rmw_gid_t gid;
  rmw_gurumdds_cpp::guid_to_gid(*guid, gid);

  if (0 == std::memcmp(gid.data, ctx->common_ctx.gid.data, 12)) {
    // compare entities' GUID prefixes to determine whether they belong to the
    // same participat (hence 12 instead of 16 bytes) Ignore own announcements
    return RMW_RET_OK;
  }
