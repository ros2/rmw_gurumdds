...
static dds_Condition * rmw_attach_reader_to_waitset(
...
static void rmw_gurumdds_listener_thread(rmw_context_impl_t *ctx)
{
  RCUTILS_LOG_DEBUG_NAMED(RMW_GURUMDDS_ID, "[listener thread] starting up...");

  auto sub_partinfo = reinterpret_cast<rmw_gurumdds_cpp::SubscriberInfo *>(
    ctx->common_ctx.sub->data);
  dds_ReturnCode_t ret = dds_RETCODE_ERROR;

  uint32_t active_len = 0;
  uint32_t attached_condition_count = 0;
  dds_Duration_t timeout = {dds_DURATION_INFINITE_SEC,
  ...
  dds_GuardCondition *gcond_exit = reinterpret_cast<dds_GuardCondition *>(
    ctx->common_ctx.listener_thread_gc->data);
  ...
  if (sub_partinfo->topic_reader != nullptr) {
    cond_part_info = rmw_attach_reader_to_waitset(sub_partinfo->topic_reader,
  ...
  if (RMW_RET_OK != dds_WaitSet_attach_condition(
                        waitset_info->wait_set,
                        reinterpret_cast<dds_Condition *>(gcond_exit)))
  {
    RMW_SET_ERROR_MSG(
        "failed to attach exit condition to listener thread waitset");
    goto cleanup;
  }
  attached_exit = true;
  attached_condition_count += 1;
  ...
  waitset_info->active_conditions =
    raii::dds_ConditionSeq_create(attached_condition_count);
  ...
  do {
    ...
    ret = dds_WaitSet_wait(waitset_info->wait_set,
                           waitset_info->active_conditions, &timeout);
      ...
      if (attached_exit) {
        if (dds_RETCODE_OK !=
          dds_WaitSet_detach_condition(
                ...
                reinterpret_cast<dds_Condition *>(gcond_exit)))
        {
          RMW_SET_ERROR_MSG("failed to detach graph condition from "
                            "listener thread waitset");
          return;
        }
      }
      if (attached_partinfo) {
        if (dds_RETCODE_OK != dds_WaitSet_detach_condition(
                                  waitset_info->wait_set, cond_part_info))
        {
          RMW_SET_ERROR_MSG("failed to detach participant info condition from "
                            "listener thread waitset");
          return;
        }
        dds_DataReader_delete_readcondition(
            sub_partinfo->topic_reader,
            reinterpret_cast<dds_ReadCondition *>(cond_part_info));
      }
...
rmw_ret_t run_listener_thread(rmw_context_t *ctx)
{
  ...
  try {
    common_ctx->listener_thread =
      std::thread(rmw_gurumdds_listener_thread, ctx->impl);
  ...
  if (rmw_destroy_guard_condition(common_ctx->listener_thread_gc) !=
    RMW_RET_OK)
  {
    RCUTILS_LOG_ERROR_NAMED(
        RMW_GURUMDDS_ID, "Failed to destroy listener thread guard condition");
  }
...
rmw_ret_t stop_listener_thread(rmw_context_t *ctx)
{
  ...
  if (common_ctx->thread_is_running.exchange(false)) {
    if (rmw_trigger_guard_condition(common_ctx->listener_thread_gc) !=
      RMW_RET_OK)
    {
      return RMW_RET_ERROR;
    }
