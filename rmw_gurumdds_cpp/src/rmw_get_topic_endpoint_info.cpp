...
{
  CHECK_ALL_PTRS_CODE(node, topic_name, publishers_info);
  ...
  rmw_gurumdds_cpp::DemangleFunction demangle_type =
    rmw_gurumdds_cpp::identity_demangle;
  if (!no_mangle) {
    mangled_topic_name = rmw_gurumdds_cpp::create_topic_name(
...
{
  CHECK_ALL_PTRS_CODE(node, allocator, topic_name, subscriptions_info);
  RCUTILS_CHECK_ALLOCATOR_WITH_MSG(
    allocator, "allocator is null", return RMW_RET_INVALID_ARGUMENT);
  CHECK_ID_CODE(node);
