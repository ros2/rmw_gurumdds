...
{
  context.registry =
    std::make_unique<rosidl_buffer_backend_registry::BufferBackendRegistry>();
  ...
  RCUTILS_LOG_DEBUG_NAMED(LOGGERNAME, "Buffer backends: found %zu backend(s)",
                          backend_names.size());

  for (const auto & backend_name : backend_names) {
    auto backend = registry.create_backend_instance(backend_name);
    if (!backend) {
      RCUTILS_LOG_ERROR_NAMED(LOGGERNAME, "Backend '%s' pointer is null",
                              backend_name.c_str());
      continue;
    }

    std::string backend_type = backend->get_backend_type();
    RCUTILS_LOG_DEBUG_NAMED(LOGGERNAME, "Processing backend '%s' (type: %s)",
    ...
    ops.create_descriptor_with_endpoint =
      [backend_ptr](const void *impl,
      const rmw_topic_endpoint_info_t & endpoint_info)
      -> std::shared_ptr<void> {
        return backend_ptr->create_descriptor_with_endpoint(impl, endpoint_info);
      };
    ops.from_descriptor_with_endpoint =
      [backend_ptr](const void *descriptor,
      const rmw_topic_endpoint_info_t & endpoint_info)
      -> std::unique_ptr<void, void (*)(void *)> {
        return backend_ptr->from_descriptor_with_endpoint(descriptor,
                                                        endpoint_info);
      };
    ...
    if (!descriptor_ts) {
      RCUTILS_LOG_ERROR_NAMED(
          LOGGERNAME, "  Backend '%s' returned null descriptor type support",
          backend_type.c_str());
      continue;
    }
    const rosidl_message_type_support_t *fastrtps_descriptor_ts =
    ...
    if (!callbacks) {
      RCUTILS_LOG_ERROR_NAMED(LOGGERNAME,
    ...
    desc_ser.serialize =
      [callbacks](
      eprosima::fastcdr::Cdr & cdr, const std::shared_ptr<void> & desc_ptr,
      const rmw_topic_endpoint_info_t & endpoint_info,
    ...
    desc_ser.deserialize =
      [callbacks, backend_ptr_for_desc](
      eprosima::fastcdr::Cdr & cdr,
      const rmw_topic_endpoint_info_t & endpoint_info,
      const rosidl_typesupport_fastrtps_cpp::BufferSerializationContext
      & serialization_context) -> std::shared_ptr<void> {
    ...
    RCUTILS_LOG_DEBUG_NAMED(
        LOGGERNAME, "  FastCDR descriptor serializers registered for '%s'",
        backend_type.c_str());
  }
}
...
void shutdown_buffer_backends(BufferBackendContext & context)
{
  try {
    ...
    RCUTILS_LOG_ERROR_NAMED(
        LOGGERNAME, "Error during buffer backend shutdown: %s", e.what());
  }
}
