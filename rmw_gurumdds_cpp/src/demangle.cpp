...
{
std::string demangle_if_ros_topic(const std::string & topic_name)
{
  return strip_ros_prefix_if_exists(topic_name);
}

std::string demangle_if_ros_type(const std::string & dds_type_string)
{
...
std::string demangle_ros_topic_from_topic(const std::string & topic_name)
{
  ...
  std::string suffix)
{
  ...
  if (suffix_position != std::string::npos) {
    if (service_name.length() - suffix_position - suffix.length() != 0) {
      RCUTILS_LOG_WARN_NAMED(RMW_GURUMDDS_ID,
                             "service topic has prefix and suffix,"
                             "but not at the end : '%s'",
                             topic_name.c_str());
      return "";
    }
  } else {
...
std::string demangle_service_type_only(const std::string & dds_type_name)
{
...
std::string identity_demangle(const std::string & name) {return name;}
} // namespace rmw_gurumdds_cpp
